...
describe("RandomGenerator", () => {
	...
	describe("constructor", () => {
		...
		it("should reject very large positive seeds", () => {
			expect(() => new RandomGenerator(4000000000)).toThrow("initialSeed must be between 1 and 2147483647.");
		});
		...
		it("should reject problematic seed values that caused database errors", () => {
			expect(() => new RandomGenerator(2231509709)).toThrow("initialSeed must be between 1 and 2147483647.");
		});
	});
	...
	describe("nextIntInRange", () => {
		...
		it("should throw error when min is greater than max", () => {
			expect(() => generator.nextIntInRange(10, 5)).toThrow("min cannot be greater than max");
		});
		...
		it("should generate deterministic sequence with same seed", () => {
			...
			const sequence1 = Array.from({ length: 10 }, () => gen1.nextIntInRange(1, 100));
			const sequence2 = Array.from({ length: 10 }, () => gen2.nextIntInRange(1, 100));
	...
	describe("nextBoolean", () => {
		...
		it("should generate deterministic sequence with same seed", () => {
			...
			const sequence1 = Array.from({ length: 10 }, () => gen1.nextBoolean());
			const sequence2 = Array.from({ length: 10 }, () => gen2.nextBoolean());

			expect(sequence1).toEqual(sequence2);
		});

		it("should generate both true and false values over many iterations", () => {
			const results = Array.from({ length: 1000 }, () => generator.nextBoolean());
	...
	describe("skip", () => {
		...
		it("should throw error for negative rounds", () => {
			expect(() => generator.skip(-1)).toThrow("rounds must be a non-negative integer number.");
		});

		it("should throw error for non-integer rounds", () => {
			expect(() => generator.skip(1.5)).toThrow("rounds must be a non-negative integer number.");
		});
	...
	describe("setState", () => {
		...
		it("should throw error for invalid state with non-number seed", () => {
			expect(() => generator.setState({ seed: "invalid" as unknown as number })).toThrow(
				"Invalid state: seed must be an integer number.",
			);
		});

		it("should throw error for invalid state with non-integer seed", () => {
			expect(() => generator.setState({ seed: 1.5 })).toThrow("Invalid state: seed must be an integer number.");
		});
	});
	...
	describe("fromDatabaseSeed", () => {
		...
		it("should throw error for invalid database seed", () => {
			expect(() => new RandomGenerator(0)).toThrow("initialSeed must be between 1 and 2147483647.");
			expect(() => new RandomGenerator(2147483648)).toThrow("initialSeed must be between 1 and 2147483647.");
		});

		it("should throw error for non-integer database seed", () => {
			expect(() => new RandomGenerator(1.5)).toThrow("initialSeed must be an integer number.");
		});

		it("should throw error for non-number database seed", () => {
			expect(() => new RandomGenerator("123" as unknown as number)).toThrow(
