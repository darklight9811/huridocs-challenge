...
export interface MockGenerator<T> {
	 ...
	 */
	seeded<Count extends number = 1>(seed: number, count?: Count, overrides?: Partial<T>): Count extends 1 ? T : T[];
}
 ...
 */
export function createMockGenerator<T>(config: { generator: (options: { faker: typeof faker; seed?: number }) => T }) {
	...
	return {
		...
		seeded: <Count extends number = 1>(seed: number, count?: Count, overrides = {}): Count extends 1 ? T : T[] => {
			if (count === undefined || count === 1) {
				faker.seed(seed);
				const result = generator({ faker, seed });
				return deepMerge<T>(result, overrides as Partial<T>) as Count extends 1 ? T : T[];
			}
